...
public class Main {
	...
	public Main() {
		...
		while(glfwWindowShouldClose(window) != true) {
			...
			glClear(GL_COLOR_BUFFER_BIT);//очищення вікна,його залиття кольором, вказаним у glClearColor
			
			shader.bind();
			shader.setUniform("sampler", 0);//шейдер використовуватиме текстуру з нульової комірки (всього 32)
			tex.bind(0);					//заливаюмо текстуру в нульову комірку
			model.render();
